...
场景一，当路由`/a`变为`/b`后，将由页面 A 跳转至页面 B。此时页面 A 已经有了填写一半的 Form 表单数据，并且我希望再次从路由`/b`切回`/a`的时候，表单数据依然存在。

场景二，有一个商品的长列表页，各个商品数据通过 lazyLoad 加载。点击某个商品，进入详情页。当我返回到列表页，我希望所有的商品列表数据依然在，同时滚动条处在原处。

本库就是专注于以上两种场景，用较低的成本，来实现这个事儿～
